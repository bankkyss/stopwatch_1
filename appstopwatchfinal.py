...
app = dash.Dash(__name__, meta_tags=[{"name": "viewport", "content": "width=device-width"}],title="stopwatch to csv")

app.layout=app.layout = html.Div(
    children=[
            html.Div(children=[
                html.H1('''stopwatch''',style={'padding-top': '20px'})
            ]),
            html.Div(
                children=[
                    html.H1('', id='client-time',style={'font-size': '90px'}),
            ...
            html.Div(
                ...
                html.Div(
                    children=[
                        ...
                        html.Div(
                            children=[
                                html.Button(children='lap', id='lap_button',n_clicks_timestamp=0, style=green_button_style),
                                html.Button(children='reset', id='reset_button',n_clicks_timestamp=0, style=red_button_style)
                            ],style={'padding-top': '5px','width': '50%','text-align': 'center',}
                        ),
                        html.Div(
                            children=[
                                html.Button(children='stop', id='stop_button',n_clicks_timestamp=0, style=red_button_style),
                                html.Button(children="Start", id="start_button",n_clicks_timestamp=0,style=green_button_style)
                            ],style={'padding-top': '5px','width': '50%','text-align': 'center',}
                        )
                ],style={"display": 'flex','flex-direction': 'row','padding-top': '5px'}
                )
            ]),
            
            html.Div(
                children=[
                    html.Table(id='datarow',style={'width': '100%'})
                    ]
            ),
            html.Div(
                children=[
                    html.Button("Download CSV", id="submit-val"),
    ...
    ],style={'width': '55%','margin-left': 'auto','margin-right': 'auto','min-width': '500px'}
 )
...
def loaddata(n_clicks,data):
    ...
    return dcc.send_data_frame(df.to_csv, "stopwatch.csv")

@app.callback(Output('submit-val','style'),[Input('stop_button','n_clicks_timestamp'),Input('start_button','n_clicks_timestamp'),Input('reset_button','n_clicks_timestamp')])
